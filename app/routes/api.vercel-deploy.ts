...
async function injectSupabaseEnvVars(
  token: string,
  projectId: string,
  ...
  const entries = [
    { key: 'VITE_SUPABASE_URL', value: supabaseEnv.url },
    { key: 'VITE_SUPABASE_ANON_KEY', value: supabaseEnv.anonKey },
  ];
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    const { projectId, files, sourceFiles, token, chatId, framework, supabaseEnv } =
    ...
    if (targetProjectId && supabaseEnv) {
      await injectSupabaseEnvVars(token, targetProjectId, supabaseEnv);
    }
