...
import { resolveOpenAIChatModel } from '~/lib/modules/llm/openai-chat-models';
...
export async function streamText(props: {
  ...
  if (provider.name === 'OpenAI') {
    const safeModel = resolveOpenAIChatModel(modelsList, currentModel);

    if (!safeModel) {
      throw new Error(`No supported OpenAI chat model available. Select gpt-4o or gpt-4o-mini in the model picker.`);
    }

    if (!modelDetails || modelDetails.name !== safeModel.name) {
      logger.warn(`MODEL [${currentModel}] is not valid for code generation. Using ${safeModel.name} instead.`);
      modelDetails = safeModel;
      currentModel = safeModel.name;
    }
  ...
  if (chatMode === 'build' && contextFiles && contextOptimization) {
    ...
    if (summary) {
      ...
      // messageSliceId 0 is valid (keep all messages) — do not treat 0 as falsy
      if (props.messageSliceId != null && props.messageSliceId > 0) {
        processedMessages = processedMessages.slice(props.messageSliceId);
      } else if (props.messageSliceId === 0) {
        // e.g. template import flow: user spec + assistant artifact + continue prompt — keep full history
      } else {
