...
const SETTINGS_KEYS = {
  ...
  FULL_STACK_ORCHESTRATION: 'enableFullStackOrchestration',
  AUTO_EXECUTE_SUPABASE: 'autoExecuteSupabaseQueries',
} as const;
...
const getInitialSettings = () => {
  ...
  const enableFullStackOrchestration = getStoredBoolean(SETTINGS_KEYS.FULL_STACK_ORCHESTRATION, true);
  ...
  return {
    ...
    promptId: isBrowser
      ? localStorage.getItem(SETTINGS_KEYS.PROMPT_ID) || (enableFullStackOrchestration ? 'fullstack-auto' : 'default')
      : enableFullStackOrchestration
        ? 'fullstack-auto'
        : 'default',
    developerMode: getStoredBoolean(SETTINGS_KEYS.DEVELOPER_MODE, false),
    enableFullStackOrchestration,
    autoExecuteSupabaseQueries: getStoredBoolean(SETTINGS_KEYS.AUTO_EXECUTE_SUPABASE, true),
  };
};
...
export const enableFullStackOrchestrationStore = atom<boolean>(initialSettings.enableFullStackOrchestration);
export const autoExecuteSupabaseQueriesStore = atom<boolean>(initialSettings.autoExecuteSupabaseQueries);
...
export const updateFullStackOrchestration = (enabled: boolean) => {
  enableFullStackOrchestrationStore.set(enabled);
  localStorage.setItem(SETTINGS_KEYS.FULL_STACK_ORCHESTRATION, JSON.stringify(enabled));
};

export const updateAutoExecuteSupabaseQueries = (enabled: boolean) => {
  autoExecuteSupabaseQueriesStore.set(enabled);
  localStorage.setItem(SETTINGS_KEYS.AUTO_EXECUTE_SUPABASE, JSON.stringify(enabled));
};
