...
import { isOpenAIChatCompletionModel } from '~/lib/modules/llm/openai-chat-models';
...
export default class OpenAIProvider extends BaseProvider {
  ...
  async getDynamicModels(
    ...
    const data = res.data.filter(
      (model: any) =>
        model.object === 'model' && isOpenAIChatCompletionModel(model.id) && !staticModelIds.includes(model.id),
    );
  ...
  getModelInstance(options: {
    ...
    if (!isOpenAIChatCompletionModel(model)) {
      throw new Error(
        `Model "${model}" is not supported for chat/code generation. Choose gpt-4o or gpt-4o-mini in the model selector.`,
      );
    }
