...
import {
  ...
  enableFullStackOrchestrationStore,
  autoExecuteSupabaseQueriesStore,
  ...
  updateEventLogs,
  updatePromptId,
  updateFullStackOrchestration,
  updateAutoExecuteSupabaseQueries,
...
export interface UseSettingsReturn {
  ...
  enableFullStackOrchestration: boolean;
  setEnableFullStackOrchestration: (enabled: boolean) => void;
  autoExecuteSupabaseQueries: boolean;
  setAutoExecuteSupabaseQueries: (enabled: boolean) => void;
...
export function useSettings(): UseSettingsReturn {
  ...
  const enableFullStackOrchestration = useStore(enableFullStackOrchestrationStore);
  const autoExecuteSupabaseQueries = useStore(autoExecuteSupabaseQueriesStore);
  ...
  const setEnableFullStackOrchestration = useCallback((enabled: boolean) => {
    updateFullStackOrchestration(enabled);
    logStore.logSystem(`Full-stack orchestration ${enabled ? 'enabled' : 'disabled'}`);
  }, []);

  const setAutoExecuteSupabaseQueries = useCallback((enabled: boolean) => {
    updateAutoExecuteSupabaseQueries(enabled);
    logStore.logSystem(`Auto-execute Supabase queries ${enabled ? 'enabled' : 'disabled'}`);
  }, []);
