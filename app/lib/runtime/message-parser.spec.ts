...
describe('EnhancedStreamingMessageParser', () => {
  ...
  it('should not duplicate plain text when streaming incremental chunks', () => {
    const parser = new EnhancedStreamingMessageParser();
    const chunks = ["Let's", ' fully', ' implement', ' the', ' app'];
    let message = '';
    let displayed = '';

    // Parser now returns the cumulative text; callers REPLACE rather than append.
    for (const chunk of chunks) {
      message += chunk;
      displayed = parser.parse('stream-1', message, true);
    }

    expect(displayed).toBe("Let's fully implement the app");
  });
...
\`\`\``;
    ...
    describe('GPT-4o style outputs', () => {
      it('should auto-wrap a code block whose first line is a plain path comment', () => {
        const input = `Sure, let's wire up the Supabase client.
...
\`\`\``;
        ...
        parser.parse('test_gpt4o_firstline', input);
      ...
      it('should auto-wrap multiple first-line-comment blocks in one message', () => {
        const input = `Here is the app shell and login page.
...
\`\`\``;
        ...
        parser.parse('test_gpt4o_multi', input);

        const fileCalls = callbacks.onActionOpen.mock.calls.filter((call: any) => call[0].action.type === 'file');
        const filePaths = fileCalls.map((call: any) => call[0].action.filePath);
        expect(filePaths).toContain('/src/App.tsx');
        expect(filePaths).toContain('/src/pages/Login.tsx');
      });
    });
...
\`\`\``;
    ...
    describe('Performance and Deduplication', () => {
      it('should handle incremental parsing correctly', () => {
        /*
         * Parse incrementally (simulating streaming). Enhancement is now deferred until
         * the final non-streaming call so the parser stays O(n) total during streaming.
         */
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          parser.parse('test_perf_1', fullInput, !isLastChunk);
        }
      ...
      it('should handle streaming input correctly', () => {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          const isLastChunk = i === chunks.length - 1;
          parser.parse('test_stream_1', fullInput, !isLastChunk);
        }
