...
import { autoExecuteSupabaseQueriesStore } from '~/lib/stores/settings';
  ...
  async handleSupabaseAction(action: SupabaseAction) {
    ...
    switch (operation) {
      ...
      case 'query': {
        ...
        const autoExecute = autoExecuteSupabaseQueriesStore.get();

        if (autoExecute && connection.token && connection.selectedProjectId && content) {
          try {
            ...
            this.onSupabaseAlert?.({
              ...
              description: 'Query executed automatically',
              content,
              source: 'supabase',
            });
            ...
            logger.error('Auto Supabase query failed', error);
          }
        }
