...
import { SystemCapabilities } from '~/components/chat/SystemCapabilities';
...
export default function FeaturesTab() {
  const {
    ...
    setEnableFullStackOrchestration,
    autoExecuteSupabaseQueries,
    setAutoExecuteSupabaseQueries,
  } = useSettings();
  ...
  React.useEffect(() => {
    ...
    if (enableFullStackOrchestration === undefined) {
      setEnableFullStackOrchestration(true);
    }

    if (autoExecuteSupabaseQueries === undefined) {
      setAutoExecuteSupabaseQueries(true);
    }
  ...
  const handleToggleFeature = useCallback(
    (id: string, enabled: boolean) => {
      switch (id) {
        ...
        case 'fullStackOrchestration': {
          setEnableFullStackOrchestration(enabled);
          toast.success(`Full-stack orchestration ${enabled ? 'enabled' : 'disabled'}`);
          break;
        }

        case 'autoExecuteSupabase': {
          setAutoExecuteSupabaseQueries(enabled);
          toast.success(`Auto-run Supabase SQL ${enabled ? 'enabled' : 'disabled'}`);
          break;
        }
  ...
  const features = {
    stable: [
      ...
      {
        id: 'fullStackOrchestration',
        title: 'Full-Stack Orchestration',
        description: 'Expand short prompts and auto-wire Supabase before building',
        icon: 'i-ph:rocket-launch',
        enabled: enableFullStackOrchestration,
        tooltip: 'Refines your request into a complete app spec with auth and database',
      },
      {
        id: 'autoExecuteSupabase',
        title: 'Auto-Run Supabase SQL',
        description: 'Execute database migrations without manual approval',
        icon: 'i-ph:database',
        enabled: autoExecuteSupabaseQueries,
        tooltip: 'When enabled, SQL from the AI runs automatically if Supabase is connected',
      },
    ],
    beta: [],
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <h3 className="text-base font-medium text-bolt-elements-textPrimary mb-1 flex items-center gap-2">
          <span className="i-ph:compass" />
          What Bolt builds well
        </h3>
        <p className="text-xs text-bolt-elements-textSecondary mb-3">
          Updates when you toggle full-stack orchestration above
        </p>
        <SystemCapabilities variant="settings" />
      </motion.div>
