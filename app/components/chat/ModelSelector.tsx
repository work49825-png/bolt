...
function getDefaultModelNameForProvider(providerName: string, modelList: ModelInfo[]): string | undefined {
  if (providerName === 'OpenAI') {
    return pickPreferredOpenAIModelName(modelList);
  }

  return modelList.find((m) => m.provider === providerName)?.name;
}
...
export const ModelSelector = ({
  ...
  const filteredModels = useMemo(() => {
    let baseModels = [...modelList].filter((e) => e.provider === provider?.name && e.name);

    if (provider?.name === 'OpenAI') {
      baseModels = baseModels.filter((m) => isOpenAIChatCompletionModel(m.name));
    }
      ...
      case 'Enter':
        ...
        if (focusedProviderIndex >= 0 && focusedProviderIndex < filteredProviders.length) {
          ...
          if (setProvider) {
            ...
            const defaultModel = getDefaultModelNameForProvider(selectedProvider.name, modelList);
  ...
  useEffect(() => {
    ...
    if (provider && !providerList.some((p) => p.name === provider.name)) {
      ...
      const defaultModel = getDefaultModelNameForProvider(firstEnabledProvider.name, modelList);
  ...
  // Image / audio OpenAI models cannot generate code — reset stale cookie selections
  useEffect(() => {
    if (provider?.name !== 'OpenAI' || !model || !setModel) {
      return;
    }

    if (isOpenAIChatCompletionModel(model)) {
      return;
    }

    const replacement = pickPreferredOpenAIModelName(modelList);

    if (replacement) {
      setModel(replacement);
      toast.info(`Switched to ${replacement} — image and audio models cannot build apps.`, { autoClose: 5000 });
    }
                  ...
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      if (setProvider) {
                        ...
                        const defaultModel = getDefaultModelNameForProvider(providerOption.name, modelList);
